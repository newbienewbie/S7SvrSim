...
internal class S7MBlock : IS7MBlock
{
    ...
    public S7MBlock(IMediator mediator, ILogger<S7DataBlockService> logger)
    {
        _mediator = mediator;
        _runningVM = Locator.Current.GetRequiredService<RunningSnap7ServerVM>();
        _loggerVM = Locator.Current.GetRequiredService<MsgLoggerVM>();
        _logger = logger;
    }
